...
#创建图
G = nx.DiGraph()
readNetwork("F:\\社交挖掘\\twitter\\12831_edges.txt")
# nx.draw(G)
...
in_degree_centrality = sorted(in_degree_centrality.items(), key=lambda d:d[0])
out_degree_centrality = sorted(out_degree_centrality.items(), key=lambda d:d[0])
...
print("in_degree_centrality: ", in_degree_centrality)
...
for row in in_degree_centrality:
    row =list(row)
    rowtxt = '{}'.format(row)
    output.write(rowtxt)
    output.write('\n')
...
for row in out_degree_centrality:
    row =list(row)
    rowtxt = '{}'.format(row)
    output.write(rowtxt)
    output.write('\n')

output = open('data4.txt','w',encoding='gbk')
...
# https://networkx.github.io/documentation/stable/auto_examples/drawing/plot_degree_histogram.html?highlight=histogram